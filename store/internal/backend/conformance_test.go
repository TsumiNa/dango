...
func backendCases() []backendCase {
	cases := []backendCase{
		...
		},
		{
			...
			open: func(t *testing.T) (persistencepkg.Backend, func()) {
				t.Helper()
				backend, err := backendpkg.NewSQLiteBackend(filepath.Join(t.TempDir(), "dango.db"))
				if err != nil {
		...
		},
		{
			...
			open: func(t *testing.T) (persistencepkg.Backend, func()) {
				...
				backend, err := backendpkg.NewPostgresBackend(dsn, filepath.Join(t.TempDir(), "workspace"))
				if err != nil {
