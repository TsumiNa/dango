...
func TestExchangeDocMarkdownRoundTrip(t *testing.T) {
	...
	if parsed.Kind != ExchangeDocKind || parsed.Version != ExchangeDocVersion {
...
func TestParseExchangeDocMarkdownRejectsLegacyExchangeKind(t *testing.T) {
	raw := `---
kind: dango.exchange
version: 1
runner_id: runner-1
...
legacy body`
	...
	if err == nil || !strings.Contains(err.Error(), "want \""+ExchangeDocKind+"\"") {
