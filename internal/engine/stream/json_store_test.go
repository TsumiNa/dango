...
func TestJSONStorePersistsReplayAcrossReopen(t *testing.T) {
	...
	stream := New(Scope{RunnerID: "run_1"}, WithStore(store), WithBufferLimit(0))
	...
	replayStream := New(Scope{RunnerID: "run_1"}, WithStore(reopenedStore), WithBufferLimit(0))
...
func TestJSONStoreLoadToleratesPartialTrailingLine(t *testing.T) {
	...
	stream := New(Scope{}, WithStore(store), WithBufferLimit(0))
	t.Cleanup(stream.Close)
