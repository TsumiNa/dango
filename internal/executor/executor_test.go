...
func TestExecutorRunWithoutHookUsesBuiltInAI(t *testing.T) {
	...
	execMode := newForTest(os.Stdout, os.Stderr, nil, sequentialLLMFactory(
...
func TestExecutorPlanWithoutHookUsesBuiltInAI(t *testing.T) {
	...
	err := newForTest(&stdout, os.Stderr, nil, staticLLMFactory([]byte(`{
...
func TestExecutorRunWithoutHookFailsWhenBuiltInAIUnavailable(t *testing.T) {
	...
	err := newForTest(os.Stdout, os.Stderr, nil, staticLLMFactory(nil, errors.New("LLM unavailable"))).Run(context.Background(), RunOptions{})
	if err == nil {
