...
这组文档描述当前 `internal/engine` 和 `internal/engine/runner` 的真实实现，而不是旧版单文件设计稿。对应的核心实现已经分散到 request、runner lifecycle、exchange 和 persistence 几个文件中，所以文档也按同样的边界拆开。

## 四个当前事实

- 外部入口是 `internal/engine/request.go` 里的 `StartRequest`。它先返回 request-scoped stream，再在后台完成 planning、runner 装配和启动。
- `internal/engine/runner` 里的 `Runner` 已经吸收旧的 managed runner 角色。现在没有单独的 `ManagedRunner` 类型，只有 bare `Start` 和 managed `StartManaged` 两种入口。
- `Executor` 不是独立运行时主体，而是一个和单个 skill 绑定的一对一代理。它负责绑定 skill runtime、补齐 node 上下文，并把 skill event stream 暴露给 runner。
- 节点之间的标准数据面是 markdown exchange document，定义在 `internal/engine/runner/exchange.go`。request stream、runner store 和 describe replay 都围绕这份文档工作。
...
flowchart TB
	...
	O[Orchestrator]
	ReqS[(request stream)]
	OS[orchestrator skill\nplan / review / replan]
	R[Runner\nplan lifecycle + DAG engine]
	E[Executor(s)\nnode proxy containers]
	SK[Skill runtime(s)]
	RunS[(runner stream)]
...
| executor / skill binding | `internal/engine/executor.go`, `internal/engine/executor_exchange.go`, `internal/engine/runner/skill_binding.go`    | 绑定 skill runtime、session reuse、exchange 输出、skill stream merge      |
...
- Executor 负责把一个 skill 放进 runner node 的执行上下文里。它自己不定义跨节点业务语义。
...
4. [Orchestrator / Executors exchange 编排图](orchestrator-executor_interative.md)：用更简明的工程视角看 `Orchestrator` 和 `Executors` 如何通过 exchange 分工协作。