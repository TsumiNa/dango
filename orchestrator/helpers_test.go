...
func mustNodeAgent(t *testing.T, node *runnerpkg.Node) *agentpkg.Agent {
	t.Helper()
	agent, ok := node.Agent.(*agentpkg.Agent)
	if !ok || agent == nil {
		t.Fatalf("node %q agent = %T, want *agentpkg.Agent", node.Id, node.Agent)
	}
	return agent
}
