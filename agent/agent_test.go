...
func TestFormatParentOutputsRendersFallback(t *testing.T) {
	got := formatParentOutputs(map[string]any{"parent-1": "fallback body"})
	if !strings.Contains(got, "### parent-1\n\nfallback body") {
		t.Fatalf("formatParentOutputs = %q, want fallback rendering", got)
	}
}
...
func TestRunnerEmitsMemoSnapshotEventForExecutionMemoSnapshots(t *testing.T) {
	...
	// Resolve symlinks (on macOS t.TempDir() sits under /tmp, a symlink to
	// /private/tmp) so the snapshot dir the runner reports matches the path
	// derived from workspaceRoot below.
